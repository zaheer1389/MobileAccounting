...
public class SlidingTabLayout extends HorizontalScrollView {
  ...
  @SuppressLint({"NewApi"})
  protected TextView createDefaultTabView(Context context, int len) {
    TextView textView = new TextView(context);
    textView.setGravity(17);
    textView.setTextColor(getResources().getColor(android.R.color.white));
    textView.setTextSize(2, HtmlUtilities.DEFAULT_FONT_SIZE);
    ...
    if (len < 4) {
      textView.setWidth(getDisplaySize(getDisplay()).x / len);
    }
    return textView;
  }
  ...
  private void populateTabStrip() {
    ...
    for (int i = 0; i < adapter.getCount(); i++) {
      ...
      if (tabView == null) {
        tabView = createDefaultTabView(getContext(), len);
      }
