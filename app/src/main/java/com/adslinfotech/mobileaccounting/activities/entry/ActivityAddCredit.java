...
public class ActivityAddCredit extends PhotoPicker implements OnItemSelectedListener, OnClickListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView((int) R.layout.activity_addcredit);
    getSupportActionBar().setTitle(getResources().getStringArray(R.array.nav_menu_items)[1]);
    getViews();
    displayResultList();
  ...
  private void setSelection() {
    try {
      int pos = this.mSpAccountAdapter.setSelection(getIntent().getExtras().getString(AppConstants.ACCOUNT_SELECTED));
      Log.e("setSelection", "pos: " + pos);
  ...
  public void insertDetails() {
    if (this.mSpAccount.getSelectedItemPosition() == 0) {
      Toast.makeText(this, getString(R.string.txt_SelectAccount), Toast.LENGTH_SHORT).show();
      return;
    }
    ...
    if (TextUtils.isEmpty(amount)) {
      ...
      Toast.makeText(getApplicationContext(), getString(R.string.txt_CreditAdded), Toast.LENGTH_SHORT).show();
      setResult(-1);
      if (this.Ismail) {
