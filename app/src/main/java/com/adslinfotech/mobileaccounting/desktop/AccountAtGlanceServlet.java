...
public class AccountAtGlanceServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    ArrayList<Transaction> mTransections = this.mFetchData.getCategoryBalance(null);
    Transaction total = (Transaction) ((ArrayList) this.mFetchData.getAccountGlance(true).get(2)).get(0);
    this.strCredit = this.newFormat.format(total.getCraditAmount()) + "/-";
    this.strDebit = this.newFormat.format(total.getDebitAmount()) + "/-";
    StringBuilder sb = new StringBuilder();
    sb.append("<!DOCTYPE html><html lang='en' class='no-js'>");
    sb.append("<head><title>Simple Accounting - Account at a Galance Report</title>");
    ...
    sb.append("</header><div class='component'><h2>Simple Accounting - Account at a Galance Report <span style='font-weight: normal;font-style: italic;'> by ADSL Infotech</span></h2>");
    ...
    sb.append("<table>\n");
    sb.append("<th>SNo</th><th>Account</th><th>Debit</th><th>Credit</th>");
    int i = 1;
    Iterator it = mTransections.iterator();
    while (it.hasNext()) {
      Transaction dao = (Transaction) it.next();
      int i2;
      if (dao.getDebitAmount() > dao.getCraditAmount()) {
        String totalbalance1 = "" + this.newFormat.format(dao.getDebitAmount() - dao.getCraditAmount());
        sb.append("<tr>\n");
        i2 = i + 1;
        sb.append("<td>" + i + "</td><td>" + dao.getAccName() + "</td><td class='right' style= background-color:#551076>" + totalbalance1 + "</td><td class='right'> </td>");
        sb.append("</tr>\n");
        i = i2;
      } else if (dao.getDebitAmount() != dao.getCraditAmount()) {
        String totalbalance2 = "" + this.newFormat.format(dao.getCraditAmount() - dao.getDebitAmount()) + "/-";
        sb.append("<tr>\n");
        i2 = i + 1;
        sb.append("<td>" + i + "</td><td>" + dao.getAccName() + "</td><td class='right'> </td><td class='right' style= background-color:#ff0000>" + totalbalance2 + "</td>");
        sb.append("</tr>\n");
        i = i2;
      }
    }
    sb.append("<tfoot style='background:#3b4259;'><tr>\n");
    sb.append(new StringBuilder("<td colspan='2' class='right'>Total:</td><td class='right' style= background-color:#551076>").append(this.strDebit).append("</td><td class='right' style= background-color:#ff0000>").append(this.strCredit));
    sb.append("</tr></tfoot>\n");
